...
| KQ-RUNTIME-002 | Original lobby render | Local patched copy + headless Chromium smoke test | SURVIVES | The authentic Key Quest lodge/room-selection screen renders in current Chrome. The private patch only bypasses the dead connection/loading gate; all `www.neopets.com` and `images.neopets.com` requests are hard-routed to local 404 sinks. This is a recovery checkpoint, not the playable alpha. |
